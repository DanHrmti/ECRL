...
class MultiHerReplayBuffer(DictReplayBuffer):
    """
    Hindsight Experience Replay (HER) buffer compatible with multiprocessing data from parallel environments
    ...
    def __init__(
        ...
        rms_normalizer: Optional = None,
    ):

        super(MultiHerReplayBuffer, self).__init__(buffer_size, env.observation_space, env.action_space, device, env.num_envs)
        ...
        self.max_episode_stored = self.buffer_size // (self.max_episode_length * self.env.num_envs)
        self.current_idx = 0
        # Counter to prevent overflow
        self.episode_steps = 0

        # Get shape and dtype of observation and goal (usually the same)
        # self.obs_shape = get_obs_shape(self.env.observation_space.spaces["observation"])  # commented out to accelerate code
        # self.obs_dtype = self.env.observation_space.spaces["observation"].dtype

        self.goal_shape = get_obs_shape(self.env.observation_space.spaces["achieved_goal"])
        self.goal_dtype = self.env.observation_space.spaces["achieved_goal"].dtype
        ...
        input_shape = {
            ...
            "done": (self.env.num_envs,) + (1,),
        }
        if self.env.smorl:
            input_shape["desired_goal"] = (self.env.num_envs,) + get_obs_shape(self.env.observation_space.spaces["desired_goal"])

        input_dtype = {
            # "observation": self.obs_dtype,
            "achieved_goal": self.goal_dtype,
            "desired_goal": self.goal_dtype,
            "action": np.float32,
            "reward": np.float32,
            # "next_obs": self.obs_dtype,
            "next_achieved_goal": self.goal_dtype,
            "next_desired_goal": self.goal_dtype,
            "done": int,
        }
        # self._observation_keys = ["observation", "achieved_goal", "desired_goal"]  # 'observation' excluded to accelerate code
        self._observation_keys = ["achieved_goal", "desired_goal"]
        self._buffer = {
            key: np.zeros((self.max_episode_stored, self.max_episode_length, *dim), dtype=input_dtype[key])
        ...
        self.episode_lengths = np.zeros([self.max_episode_stored, self.env.num_envs], dtype=np.int64)

        # other modules
        # self.obs_normalizer = RMSNormalizer(epsilon=1e-6, shape=self.env.observation_space["observation"].shape[-1])
        self.rms_normalizer = rms_normalizer
        self.latent_classifier = self.env.latent_classifier
    ...
    def sample_goals(
        ...
        :param env_indices: Environment indices to use.
        :return: Return sampled goals.
        """
        her_episode_indices = episode_indices[her_indices]
        her_env_indices = env_indices[her_indices]

        if self.goal_selection_strategy == GoalSelectionStrategy.FINAL:
            # replay with final state of current episode
            transitions_indices = self.episode_lengths[her_episode_indices, her_env_indices].reshape(-1) - 1

        elif self.goal_selection_strategy == GoalSelectionStrategy.FUTURE:
            # replay with random state which comes from the same episode and was observed after current transition
            transitions_indices = np.random.randint(
                transitions_indices[her_indices] + 1, self.episode_lengths[her_episode_indices, her_env_indices].reshape(-1)
            )

        elif self.goal_selection_strategy == GoalSelectionStrategy.EPISODE:
            # replay with random state which comes from the same episode as current transition
            transitions_indices = np.random.randint(self.episode_lengths[her_episode_indices, her_env_indices].reshape(-1))

        else:
            raise ValueError(f"Strategy {self.goal_selection_strategy} for sampling goals not supported!")

        new_goals = self._buffer["achieved_goal"][her_episode_indices, transitions_indices, her_env_indices]
        new_goal_infos = np.array(
            [
                self.info_buffer[episode_idx][transition_idx-1][env_idx]['position']
                for episode_idx, transition_idx, env_idx in zip(her_episode_indices, transitions_indices, her_env_indices)
            ]
        )

        if self.env.smorl:
            if self.env.obs_mode == 'state':
                goal_obj_indices = np.array(
                    [
                        self.info_buffer[episode_idx][transition_idx-1][env_idx]['goal_obj_index']
                        for episode_idx, transition_idx, env_idx in zip(her_episode_indices, transitions_indices, her_env_indices)
                    ]
                )

                new_goals = np.expand_dims(new_goals[np.arange(len(goal_obj_indices)), goal_obj_indices], -2)

            else:
                new_goals = get_single_goal(new_goals, self.latent_classifier, self.device, check_goal_reaching=False)

        return new_goals, new_goal_infos
    ...
    def _sample_transitions(
        ...
        if online_sampling:
            ...
            env_indices = np.random.randint(0, self.env.num_envs, batch_size)
        ...
        ep_lengths = self.episode_lengths[episode_indices, env_indices].reshape(-1)
        ...
        transitions = {key: self._buffer[key][episode_indices, transitions_indices, env_indices].copy() for key in self._buffer.keys()}

        # Convert info buffer to numpy array
        transitions["info"] = np.array(
            [
                self.info_buffer[episode_idx][transition_idx][env_idx]
                for episode_idx, transition_idx, env_idx in zip(episode_indices, transitions_indices, env_indices)
            ]
        )

        if len(her_indices) > 0:
            # sample new desired goals and relabel the transitions
            new_goals, new_goal_infos = self.sample_goals(episode_indices, her_indices, transitions_indices, env_indices)
            transitions["desired_goal"][her_indices] = new_goals
            for i in range(len(her_indices)):
                transitions["info"][her_indices[i]]["goal_pos"] = new_goal_infos[i]
            ...
            transitions["reward"][her_indices, 0] = self.env.env_method(
                ...
                indices=0,  # compute reward method is identical for each environment, accessing one env is sufficient
            )

        # normalize observations based on running statistics
        if self.rms_normalizer is not None:
            transitions["desired_goal"] = self.rms_normalizer.normalize(transitions["desired_goal"])
            transitions["achieved_goal"] = self.rms_normalizer.normalize(transitions["achieved_goal"])
            transitions["next_achieved_goal"] = self.rms_normalizer.normalize(transitions["next_achieved_goal"])
        ...
        next_observations = {
            # "observation": transitions["next_obs"],  # commented out to accelerate code
        ...
        if online_sampling:
            # next_obs = {key: self.to_torch(next_observations[key]) for key in self._observation_keys}  # commented out to accelerate code
            next_obs = {key: th.from_numpy(next_observations[key]).to(self.device) for key in self._observation_keys}

            # normalized_obs = {key: self.to_torch(observations[key]) for key in self._observation_keys}  # commented out to accelerate code
            normalized_obs = {key: th.from_numpy(observations[key]).to(self.device) for key in self._observation_keys}

            return DictReplayBufferSamples(
                observations=normalized_obs,
                # actions=self.to_torch(transitions["action"]),  # commented out to accelerate code
                actions=th.from_numpy(transitions["action"]).to(self.device),
                next_observations=next_obs,
                dones=th.from_numpy(transitions["done"]).to(self.device),
                rewards=th.from_numpy(self._normalize_reward(transitions["reward"], maybe_vec_env)).to(self.device),
            )
        else:
    ...
    def add(
        ...
        # self._buffer["observation"][self.pos][self.current_idx] = obs["observation"]  # commented out to accelerate code
        ...
        self._buffer["done"][self.pos][self.current_idx] = done_.reshape([*done_.shape, 1])
        self._buffer["reward"][self.pos][self.current_idx] = reward.reshape([*reward.shape, 1])
        # self._buffer["next_obs"][self.pos][self.current_idx] = next_obs["observation"]  # commented out to accelerate code
        ...
        if done.all() or self.episode_steps >= self.max_episode_length:
            self.store_episode()
